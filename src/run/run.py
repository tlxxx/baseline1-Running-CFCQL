...
def run(_run, _config, _log):
    ...
    if args.use_offline:
        ...
        unique_token = "{}__{}__{}__{}__{}".format(args.name,map_name,tb_suffix,args.runner, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    else:
        unique_token = "{}__{}__{}__{}".format(args.name,map_name,args.runner, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
...
def collect_sequential(args, runner):
    ...
    if os.path.exists(data_dir+'/'+map_name+'_'+args.h5file_suffix + '.h5'):
        os.remove(data_dir+'/'+map_name+'_'+args.h5file_suffix + '.h5')
    f = h5py.File(data_dir+'/'+map_name+'_'+args.h5file_suffix + '.h5', 'a')
